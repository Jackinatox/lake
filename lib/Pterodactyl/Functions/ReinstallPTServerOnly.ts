...
export async function reinstallPTServerOnly(
    ...
    logger.info(`Initiating server reinstall for ${serverId}`, 'GAME_SERVER', {
    ...
    if (!response.ok) {
        ...
        logger.error(`Failed to reinstall server ${serverId}`, 'GAME_SERVER', {
            details: {
    ...
    logger.info(`Successfully completed reinstall for server ${serverId}`, 'GAME_SERVER');
}
