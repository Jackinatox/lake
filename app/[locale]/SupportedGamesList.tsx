      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
        {supportedGames.map((game) => (
          <Link href={`/booking2/${game.id}`} key={game.id} className="block">
            <div
              className="flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-lg hover:bg-muted transition-all hover:scale-[1.02] border border-transparent hover:border-primary/20"
            >
              ...
              <span className="font-medium text-base md:text-sm flex-1">{game.name}</span>
