...
export async function updateFtpPassword({
  ...
  const finalPassword = trimmedPassword && trimmedPassword.length > 0 ? trimmedPassword : generateRandomPassword()

  if (finalPassword.length < FTP_PASSWORD_MIN_LENGTH || finalPassword.length > FTP_PASSWORD_MAX_LENGTH) {
    return {
      success: false,
