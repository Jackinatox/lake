...
export function UpgradeHardwareConfigToPayed({
    initialConfig,
    performanceOptions,
    onNext,
}: HardwareConfigProps) {
    ...
    useEffect(() => {
        if (selectedPFGroup?.cpu && selectedPFGroup?.ram) {
            const upgradeByCPU = cpuCores * 100;
            const upgradeByRAM = ramGb * 1024;

            setTotalPrice(calculateNew(selectedPFGroup, upgradeByCPU, upgradeByRAM, days));
        }
    ...
    return (
        <div className="min-h-screen bg-background md:-my-4">
            {/* Header with total price - STICKY TO TOP */}
            <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b py-4">
                <div className="w-full px-1 max-w-7xl mx-auto">
                    <div className="flex items-center justify-between gap-4">
                        <h1 className="text-xl sm:text-2xl font-bold">{t('config.title')}</h1>
