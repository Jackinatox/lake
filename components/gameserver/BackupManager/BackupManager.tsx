...
function mapBackup(raw: any): Backup {
    ...
    return {
        ...
        isSuccessful: typeof attributes.is_successful === "boolean" ? attributes.is_successful : null,
...
function BackupManager({ apiKey, server }: BackupManagerProps) {
    ...
    return (
        <Card className="space-y-6 sm:p-6 p-2">
            ...
            {backupLimit > 0 ? (
                <p className="text-sm text-muted-foreground">
                    Using {backups.length}/{backupLimit} backup slots · Total size {formatBytes(totalSize)}
                </p>
            ) : (
                <p className="text-sm text-muted-foreground">
