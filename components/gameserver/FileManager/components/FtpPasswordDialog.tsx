...
export function FtpPasswordDialog({ open, onOpenChange, serverIdentifier }: FtpPasswordDialogProps) {
    ...
    const handlePasswordSubmit = useCallback(() => {
        ...
        if (trimmedPassword.length > 0) {
            ...
            if (trimmedPassword.length < FTP_PASSWORD_MIN_LENGTH) {
                setPasswordError(`Password must be at least ${FTP_PASSWORD_MIN_LENGTH} characters.`)
                return
            }
            ...
            <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4">
                        <div className="rounded-md border bg-muted/40 p-4">
                            <div className="mb-2 flex items-center justify-between text-xs uppercase text-muted-foreground">
                                ...
                                <Button type="button" variant="ghost" size="sm" className="h-8 px-2" onClick={handlePasswordCopy}>
                                    <Copy className="mr-1 h-3.5 w-3.5" />
                                    Copy
                                </Button>
                            </div>
                            <p className="break-words font-mono text-sm" data-testid="ftp-new-password">
                                {passwordResult}
                            </p>
                        </div>
                        <p className="text-xs text-muted-foreground">
                            This password is only shown once. Store it somewhere safe before closing the dialog.
                        </p>
                    </div>
                ) : (
                    <div className="space-y-4">
                ...
                <DialogFooter>
                    {passwordResult ? (
                        ...
                        <>
                            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={isUpdatingPassword}>
                                Cancel
                            </Button>
