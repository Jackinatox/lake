...
export const HardwareConfigComponent = forwardRef(
  ...
  const [selectedPFGroup, setSelectedPFGroup] =
    ...
    useEffect(() => {
      if (initialConfig && performanceOptions.length > 0) {
        const group = performanceOptions.find(
          (pf) => pf.id === initialConfig.pfGroupId
        );
        if (group) {
    ...
    useEffect(() => {
      ...
      if (selectedPFGroup?.cpu && selectedPFGroup?.ram) {
        setTotalPrice(
          calculateNew(selectedPFGroup, cpuCores * 100, ramGb * 1024, days)
        );
      }
      ...
      totalPrice.discount.cents > 0 && totalResourceCents > 0
        ? Math.round(
            totalPrice.discount.cents *
              (totalPrice.cents.cpu / totalResourceCents)
          )
        : 0;
              ...
              <CardContent className="space-y-4 sm:space-y-6">
                    ...
                    <TabsList className="grid grid-cols-2 sm:grid-cols-4 gap-1 sm:gap-2 h-auto p-1 bg-muted/50">
                      ...
                      >
                        <div className="text-center">
                          <div className="font-medium">{t("durations.week")}</div>
                          <div className="text-xs opacity-80">{t("durations.days", { days: 7 })}</div>
                        </div>
                      </TabsTrigger>
                      <TabsTrigger
                        value="30"
                      ...
                      >
                        <div className="text-center">
                          <div className="font-medium">{t("durations.month")}</div>
                          <div className="text-xs opacity-80">{t("durations.days", { days: 30 })}</div>
                        </div>
                      </TabsTrigger>
                      <TabsTrigger
                        value="90"
                      ...
                      >
                        <div className="text-center">
                          <div className="font-medium">{t("durations.threeMonths")}</div>
                      ...
                      >
                        <div className="text-center">
                          <div className="font-medium">{t("durations.sixMonths")}</div>
                          <div className="text-xs opacity-80 text-green-600">
                            -15%
                          </div>
                        </div>
                ...
                <div className="space-y-3">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-0">
                    ...
                    <div className="text-xs sm:text-sm text-muted-foreground">
                      {(selectedPFGroup.cpu.pricePerCore / 100).toFixed(2)} {tl("perVcore")}
                    </div>
                  </div>
                  <div className="px-2">
                    <Slider
                ...
                <div className="space-y-3">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-0">
                    ...
                    <div className="text-xs sm:text-sm text-muted-foreground">
                      {(selectedPFGroup.ram.pricePerGb / 100).toFixed(2)} {tl("perGiB")}
                    </div>
                  </div>
                  <div className="px-2">
                    <Slider
