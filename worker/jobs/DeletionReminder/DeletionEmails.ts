...
parentPort?.on("message", (msg) => {
    if (msg?.type === "stop") {
        ...
        logWarn(WorkerJobType.GENERATE_DELETION_EMAILS, "Job received stop signal", null, { jobRun }).catch(console.error);
        shutdown = true;
    }
});
...
(async () => {
    try {
        ...
        await logInfo(WorkerJobType.GENERATE_DELETION_EMAILS, `Job started, processing ${count} servers`, {
            totalServers: count,
            jobRun
        }, { jobRun });
        ...
        const expiryThreshold1DayMin = new Date(now.getTime() - (DELETE_GAMESERVER_AFTER_DAYS - 1) * 24 * 60 * 60 * 1000);
        const expiryThreshold1DayMax = new Date(twoDaysFromNow.getTime() - DELETE_GAMESERVER_AFTER_DAYS * 24 * 60 * 60 * 1000);
        ...
        const expiryThreshold7DayMin = new Date(sixDaysFromNow.getTime() - DELETE_GAMESERVER_AFTER_DAYS * 24 * 60 * 60 * 1000);
        const expiryThreshold7DayMax = new Date(eightDaysFromNow.getTime() - DELETE_GAMESERVER_AFTER_DAYS * 24 * 60 * 60 * 1000);
        ...
        await logInfo(WorkerJobType.GENERATE_DELETION_EMAILS, `Job completed successfully`, {
        ...
        await logFatal(WorkerJobType.GENERATE_DELETION_EMAILS, `Job failed with critical error`, {
    ...
    await logFatal(WorkerJobType.GENERATE_DELETION_EMAILS, `Job crashed unexpectedly`, {
        error: error instanceof Error ? error.message : String(error),
        stack: error instanceof Error ? error.stack : undefined,
        processed,
        total: count,
...
async function createEmailJob(server: GameServer, days: 1 | 7, jobRun: string, deletionDate: Date) {
    try {
        ...
        await logError(WorkerJobType.GENERATE_DELETION_EMAILS, `Failed to process individual server`, {
            serverId: server.id,
