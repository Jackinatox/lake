...
export async function handleDeleted(server: GameServer, jobRun: string) {
    try {
        await logInfo(WorkerJobType.DELETE_SERVERS, `Starting to handle server deletion`, {
            serverId: server.id,
        ...
        await logInfo(WorkerJobType.DELETE_SERVERS, `Server marked as DELETED in database`, {
            serverId: server.id,
            newStatus: 'DELETED'
        }, { gameServerId: server.id, userId: server.userId, jobRun });
        ...
        await logError(WorkerJobType.DELETE_SERVERS, `Failed to handle server deletion`, {
            serverId: server.id,
...
async function deleteServer(server: GameServer, jobRun: string) {
    ...
    await logInfo(WorkerJobType.DELETE_SERVERS, `Deleting server via Pterodactyl API`, {
        serverId: server.id,
        ptAdminId: server.ptAdminId
    }, { gameServerId: server.id, userId: server.userId, jobRun });

    const response = await fetch(env.NEXT_PUBLIC_PTERODACTYL_URL + `/api/application/servers/${server.ptAdminId}`, {
        method: 'DELETE',
        headers: {
    ...
    await logInfo(WorkerJobType.DELETE_SERVERS, `Successfully deleted server via Pterodactyl API`, {
        serverId: server.id,
        ptAdminId: server.ptAdminId,
        responseStatus: response.status
    }, { gameServerId: server.id, userId: server.userId, jobRun }).catch(() => { console.log("Failed to log deletion success - THIS IS SUPER-CRITICAL") });
