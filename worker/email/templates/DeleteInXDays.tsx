...
export default function DeleteServerTemplate({ username, serverName, expirationDate, deletionDate, deletionDays, serverId }: DeleteServerTemplateProps) {
    ...
    return (
        <EmailLayout preview={previewText}>
            <Heading className="m-0 text-2xl font-bold text-slate-900">Server wird bald gelöscht</Heading>
            ...
            <Text className="mt-4 text-base leading-6 text-slate-600">
                Dein Server <span className="font-semibold text-slate-900">{serverName}</span> wird in {daysText} endgültig gelöscht, exakt am {formattedDeletionDate}.
            </Text>
            <Text className="mt-4 text-base leading-6 text-slate-600">
                Der Server ist bereits am {formattedExpirationDate} abgelaufen. Nach der Löschung können die Daten nicht wiederhergestellt werden.
            </Text>
            <Section className="mt-6 rounded-lg border border-red-200 bg-red-50 p-4">
                <Text className="m-0 text-base font-semibold text-red-900">⚠️ Löschung am: {formattedDeletionDate}</Text>
                <Text className="mt-2 text-sm leading-6 text-red-600">
                    Nach diesem Datum werden alle Serverdaten unwiderruflich gelöscht. Bitte sichere wichtige Daten oder reaktiviere den Server rechtzeitig.
                </Text>
            </Section>
